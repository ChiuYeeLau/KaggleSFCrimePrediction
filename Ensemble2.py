...
class BasicFeatures(BaseEstimator, TransformerMixin):
    ...
    def transform(self, X_df, y=None):
        ...
        if self.date_features:
            ...
            X['IsNight'] = X['Hour'].apply(self.IsNight)  
        ...
        if self.Address_features:
            ...
            X['street_type'] = X['Address'].apply(self.StreetType)
            print 'logodds!'

        if self.Interaction_feature:
            X["Interaction"] = ((X.X-X.X.mean())/X.X.std())*((X.Y-X.Y.mean())/X.Y.std())
            X[['rho','phi']] = X[['X','Y']].apply(lambda (x,y):list(self.cart2pol(x, y)),axis = 1)
            X[['X_30','Y_30']] = X[['X','Y']].apply(lambda (x,y):list(self.rotate(x,y,30)),axis=1)
            X[['X_60','Y_60']] = X[['X','Y']].apply(lambda (x,y):list(self.rotate(x,y,60)),axis=1)
            X[['X_90','Y_90']] = X[['X','Y']].apply(lambda (x,y):list(self.rotate(x,y,90)),axis=1)
...
def make_Kaggle_file(predict_probabilities, columns, output_file_name="auto", decimal_limit=3):
    ...
    if output_file_name == "auto":
        ...
        output_file_name = "SFCrimeEnsemble2.csv"
    if decimal_limit:
...
model2 = RandomForestClassifier(n_estimators=100, max_depth=8, min_samples_split=10, min_samples_leaf=10, n_jobs=-1)#, oob_score=True)
...
model3 = 'xgboost_1'
model4 = 'xgboost_2'
model5 = LogisticRegression(C=0.4)

clfs = [model3, model4, model2]
...
for i, (train_idx, test_idx) in enumerate(skf):
    ...
    for model in clfs:
        ...
        text_file.write("Done! " + str(indice) + ", " + str(i))
        text_file.close()
